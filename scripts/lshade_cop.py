#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
L-SHADE (Large-scale Self-adaptive Differential Evolution) Algorithm
with L-SHADE-COP (L-SHADE for Constraint Optimization Problems) Extension

This library uses the code obtained from PyADE: https://github.com/xKuZz/pyade/tree/master

Author: Elias J R Freitas
Date Created: 2023
Python Version: >3.8

Cite us:

L-SHADE is a self-adaptive differential evolution algorithm designed for large-scale optimization problems. It employs a dynamic population sizing strategy and a novel differential mutation scheme to improve optimization performance.
L-SHADE-COP is an extension of L-SHADE tailored for Constraint Optimization Problems (COPs). It introduces constraints handling mechanisms to address problems with constraints.

Usage:
1. Set the global variables to use ROS, SAVE datalog. 
2. Import this module into your Python script.
2. Initialize the optimization problem by defining the objective function, constraints, and other parameters.




"""
from __debug import *
from __experiment_setup import SAVE

import pyade_commons
import numpy as np
import scipy.stats
import random
from typing import Callable, Union, Any, Dict


experiment_data = []

def save_data(save=1):
    global SAVE
    if save:
        printD("Saving experiment data")
    SAVE = save



    

def get_default_params(dim: int):
    """
        Returns the default parameters of the L-SHADE Differential Evolution Algorithm.
        :param dim: Size of the problem (or individual).
        :type dim: int
        :return: Dict with the default parameters of the L-SHADE Differential
        Evolution Algorithm.
        :rtype dict
    """
    return {'max_evals': 10000 * dim, 'population_size': 18 * dim, 'individual_size': dim,
            'memory_size': 6, 'callback': None, 'seed': None, 'opts': None, 'start_line':[], 'smart_init_population':None, 'COP':False}

def apply(population_size: int, individual_size: int, bounds: np.ndarray,
          func: Callable[[np.ndarray], np.float64], opts: Any,
          memory_size: int, callback: Callable[[Dict], Any],
          max_evals: int, seed: Union[int, None], start_line, smart_init_population, COP):
    """
    Applies the L-SHADE Differential Evolution Algorithm.
    :param population_size: Size of the population.
    :type population_size: int
    :param individual_size: Number of gens/features of an individual.
    :type individual_size: int
    :param bounds: Numpy ndarray with individual_size rows and 2 columns.
    First column represents the minimum value for the row feature.
    Second column represent the maximum value for the row feature.
    :type bounds: np.ndarray
    :param func: Evaluation function. The function used must receive one
     parameter.This parameter will be a numpy array representing an individual.
    :type func: Callable[[np.ndarray], float]
    :param opts: Optional parameters for the fitness function.
    :type opts: Any type.
    :param memory_size: Size of the internal memory.
    :type memory_size: int
    :param callback: Optional function that allows read access to the state of all variables once each generation.
    :type callback: Callable[[Dict], Any]
    :param max_evals: Number of evaluations after the algorithm is stopped.
    :type max_evals: int
    :param seed: Random number generation seed. Fix a number to reproduce the
    same results in later experiments.
    :type seed: Union[int, None]
    :return: A pair with the best solution found and its fitness.
    :rtype [np.ndarray, int]
    """
    global SAVE


    # 0. Check parameters are valid
    if type(population_size) is not int or population_size <= 0:
        raise ValueError("population_size must be a positive integer.")

    if type(individual_size) is not int or individual_size <= 0:
        raise ValueError("individual_size must be a positive integer.")

    if type(max_evals) is not int or max_evals <= 0:
        raise ValueError("max_iter must be a positive integer.")

    if type(bounds) is not np.ndarray or bounds.shape != (individual_size, 2):
        raise ValueError("bounds must be a NumPy ndarray.\n"
                         "The array must be of individual_size length. "
                         "Each row must have 2 elements.")

    if type(seed) is not int and seed is not None:
        raise ValueError("seed must be an integer or None.")

    # np.random.seed(seed)
    # random.seed(seed)

    # 1. Initialization
    if smart_init_population is not None:
        printD("SMART population")
        population = smart_init_population
    else:
        population = pyade_commons.init_population(population_size, individual_size, bounds)
    if start_line is not None:
        printD(start_line, type(start_line))
        population[0] = start_line    
           
    init_size = population_size
    m_cr = np.ones(memory_size) * 0.5
    m_f = np.ones(memory_size) * 0.5
    archive = []
    archive_func = []
    k = 0


    "COP = True --> population will be divide in feasible and unfeasible solutions"
    if COP:       
        return_func = pyade_commons.apply_fitness(population, func, opts)
        return_func = np.array(return_func)
        fitness = return_func[:,0]
        return_func_fe, return_func_un, indexes_fe, indexes_un = pyade_commons.divide_feasibility(return_func)
        fitness_fe = return_func_fe[:,0]

        population_fe = population[indexes_fe]
        population_un = population[indexes_un]

        population_size_fe = len(population_fe)
        population_size_un = len(population_un)
        population_size = population_size_fe + population_size_un
        
        all_indexes = list(range(memory_size))
        current_generation = 0
        num_evals = population_size_fe + population_size_un
        # Calculate max_iters
        n = population_size_fe + population_size_un
        i = 0
        max_iters = 0
        # Calculate max_iters        
        i = 0
        max_iters = 0
        while i < max_evals:
            max_iters += 1
            n = round((4 - init_size) / max_evals * i + init_size)
            i += n
        
        # Main loop
        while num_evals < max_evals:
            # 2.1 Adaptation
            r = np.random.choice(all_indexes, population_size)
            cr = np.random.normal(m_cr[r], 0.1, population_size)
            cr = np.clip(cr, 0, 1)
            # cr[m_cr[r] == 1] = 0
            f = scipy.stats.cauchy.rvs(loc=m_f[r], scale=0.1, size=population_size)
            f[f > 1] = 1  
            while sum(f <= 0) != 0:
                r = np.random.choice(all_indexes, sum(f <= 0))
                f[f <= 0] = scipy.stats.cauchy.rvs(loc=m_f[r], scale=0.1, size=sum(f <= 0))

            f_fe = f# np.random.choice(f, population_size)
            p = np.random.uniform(0.05, 0.95, size=population_size)#np.ones(population_size) * .8
            p_fe = np.random.uniform(0.05, 0.95, size=population_size_fe)#np.ones(population_size_fe) * .8
            mutated = pyade_commons.current_to_pbest_constraints_mutation(population, population_fe, archive, \
                                                            fitness, fitness_fe, return_func, f.reshape(len(f), 1),\
                                                            f_fe.reshape(len(f_fe), 1), p, p_fe, bounds)        
            crossed = pyade_commons.crossover(population, mutated, cr.reshape(len(f), 1))
            # force value opts, to use in fitness function the current_generation
            if opts is not None:
                if len(opts) == 1:
                    opts[0] = current_generation

            c_return_func = pyade_commons.apply_fitness(crossed, func, opts)
            c_return_func = np.array(c_return_func)
            c_fitness = c_return_func[:,0]
            num_evals += population_size

            old_population = population.copy()
            old_return_func = return_func.copy()
            population, indexes = pyade_commons.selection_constraints(population, crossed,
                                                        return_func, c_return_func, return_indexes=True)
            # add to archive the discarted elements
            archive.extend(old_population[indexes])
            list_of_lists = [list(array) for array in old_return_func[indexes]]
            archive_func.extend(list_of_lists)
            
            # 2.3 Adapt for next generation            
            if len(indexes) > 0:
                if len(archive) > init_size/12: 
                    archive_func = np.array(archive_func)
                    sorted_indices_a = np.lexsort((archive_func[:, 2], archive_func[:, 0]))
                    sorted_indices_a = sorted_indices_a[0:init_size]  
                    archive = list(np.array(archive)[sorted_indices_a])
                    archive_func = [list(array) for array in archive_func[sorted_indices_a]]  

                weights = np.abs(fitness[indexes] - c_fitness[indexes])
                weights /= np.sum(weights+1e-18)
                m_cr[k] = np.sum(weights * cr[indexes] ** 2) / np.sum(weights * cr[indexes]+1e-18)
                if np.isnan(m_cr[k]):
                    m_cr[k] = 1
                m_f[k] = np.sum(weights * f[indexes] ** 2) / np.sum(weights * f[indexes]+1e-18)
                k += 1
                if k == memory_size:
                    k = 0
            
            return_func[indexes] = c_return_func[indexes].copy()
            fitness[indexes] = c_fitness[indexes].copy()
            
            # Adapt population size
            sorted_indices = np.lexsort((return_func[:, 0], return_func[:, 2]))
            new_population_size = round((4 - init_size) / max_evals * num_evals + init_size)
            if (population_size > new_population_size):# and (population_size_fe > 0):                
                population_size = new_population_size
                sorted_indices = sorted_indices[0:population_size] 
                if k == init_size:
                    k = 0
            
            fitness = fitness[sorted_indices]
            population = population[sorted_indices]
            return_func = return_func[sorted_indices]
            return_func_fe, return_func_un, indexes_fe, indexes_un = pyade_commons.divide_feasibility(return_func)
            fitness_fe = return_func_fe[:,0]            

            population_fe = population[indexes_fe]
            population_un = population[indexes_un]

            population_size_fe = len(population_fe)
            population_size_un = len(population_un)
            
            
            
            if callback is not None:
                callback(**(locals()))
            current_generation += 1
            _, _, population_size_fe, best_solution, best_fitness, best_constraint = pyade_commons.sort_indiviuals(population, return_func)   
            
            if SAVE:
                # _, _, population_size_fe, best_solution, best_fitness, best_constraint = pyade_commons.sort_indiviuals(population, return_func)
                experiment_data.append([best_fitness, np.mean(fitness), best_constraint, np.mean(return_func[:, 2])])
            
            printD(f"\rNum_evals LSHADE-COP: {num_evals}, gen:{current_generation}, s:{population_size}, sfe:{population_size_fe}, f:{best_fitness:.4f}, mf:{np.mean(fitness):.4f}                  ",end="")#, p:{population[best]}", end='')  


    else:
        return_func = pyade_commons.apply_fitness(population, func, opts)
        return_func = np.array(return_func)
        fitness = return_func[:,0]
        # fitness = pyade_commons.apply_fitness(population, func, opts)

        all_indexes = list(range(memory_size))
        current_generation = 0
        num_evals = population_size
        # Calculate max_iters
        n = population_size
        i = 0
        max_iters = 0

        # Calculate max_iters
        n = population_size
        i = 0
        max_iters = 0
        while i < max_evals:
            max_iters += 1
            n = round((4 - init_size) / max_evals * i + init_size)
            i += n

        while num_evals < max_evals:
            # 2.1 Adaptation
            r = np.random.choice(all_indexes, population_size)
            cr = np.random.normal(m_cr[r], 0.1, population_size)
            cr = np.clip(cr, 0, 1)
            cr[m_cr[r] == 1] = 0
            f = scipy.stats.cauchy.rvs(loc=m_f[r], scale=0.1, size=population_size)
            f[f > 1] = 0
            
            while sum(f <= 0) != 0:
                r = np.random.choice(all_indexes, sum(f <= 0))
                f[f <= 0] = scipy.stats.cauchy.rvs(loc=m_f[r], scale=0.1, size=sum(f <= 0))

            p = np.ones(population_size) * .10#0.11
            
            # 2.2 Common steps
            mutated = pyade_commons.current_to_pbest_archive_mutation(population, archive, fitness, f.reshape(len(f), 1), p, bounds)        
            crossed = pyade_commons.crossover(population, mutated, cr.reshape(len(f), 1))
            # force value opts, to use in fitness function the current_generation
            if opts is not None:
                if len(opts) == 1:
                    opts[0] = current_generation

            c_return_func = pyade_commons.apply_fitness(crossed, func, opts)
            c_return_func = np.array(c_return_func)
            c_fitness = c_return_func[:,0]
            num_evals += population_size

            
            population, indexes = pyade_commons.selection(population, crossed,
                                                        fitness, c_fitness, return_indexes=True)

            # 2.3 Adapt for next generation
            archive.extend(population[indexes])

            if len(indexes) > 0:
                if len(archive) > init_size*5: #modified from population_size
                    archive = random.sample(archive, init_size*5)

                weights = np.abs(fitness[indexes] - c_fitness[indexes])
                weights /= np.sum(weights+1e-18)
                m_cr[k] = np.sum(weights * cr[indexes] ** 2) / np.sum(weights * cr[indexes]+1e-18)
                if np.isnan(m_cr[k]):
                    m_cr[k] = 1
                m_f[k] = np.sum(weights * f[indexes] ** 2) / np.sum(weights * f[indexes]+1e-18)
                k += 1
                if k == memory_size:
                    k = 0

            fitness[indexes] = c_fitness[indexes]
                
            new_population_size = round((4 - init_size) / max_evals * num_evals + init_size)
            if population_size > new_population_size:
                population_size = new_population_size
                best_indexes = np.argsort(fitness)[:population_size]
                population = population[best_indexes]
                fitness = fitness[best_indexes]
                return_func = return_func[best_indexes]
                if k == init_size:
                    k = 0
            
            if callback is not None:
                callback(**(locals()))
            current_generation += 1
            
            if SAVE:
                experiment_data.append([fitness[0], np.mean(fitness), return_func[0, 2], np.mean(return_func[:, 2])])
            
            
            printD(f"\rNum_evals lshade: {num_evals}, gen:{current_generation}, s:{population_size}, f:{fitness[0]:.4f}, mf:{np.mean(fitness):.4f}                  ",end='')#, p:{population[best]}", end='')  
            
        
    
    _, _, population_size_fe, best_solution, best_fitness, best_constraint = pyade_commons.sort_indiviuals(population, return_func)        
    
    
    return best_solution, best_fitness, experiment_data
    


      